...
type Options struct {
	...
	// LintLayers flags architecture smells on call edges.
	LintLayers bool
...
| `Extractors`  | net/http, gin, echo, gRPC, GraphQL | Replace to add a framework. See [Frameworks](/docs/frameworks). |
...
| `DisableAutoLayer` | `false`     | Auto-layer is on by default; set true for keyword-only. See [Auto-layer](/docs/auto-layer). |
| `LintLayers` | `false`           | Flag reverse/skip/cross-module call edges. See [Layer linting](/docs/linting). |
